...
<body>
    ...
    getTableContent() {
        ...
        // Add Panel Amps (240V) column back for printing
        // First, add the header
        const headerRow = tableClone.querySelector('thead tr');
        if (headerRow) {
            const descriptionHeader = headerRow.cells[1]; // Description is at index 1
            if (descriptionHeader) {
                const panelAmpsHeader = document.createElement('th');
                panelAmpsHeader.textContent = 'Panel Amps (240V)';
                headerRow.insertBefore(panelAmpsHeader, descriptionHeader.nextSibling);
            }
        }
        
        // Then, add the Panel Amps cell for each data row
        const dataRows = tableClone.querySelectorAll('tbody tr');
        dataRows.forEach(row => {
            const descriptionCell = row.cells[1]; // Description is at index 1
            if (descriptionCell) {
                // Get the panel amps from the product data
                const dropdown = descriptionCell.querySelector('select');
                let panelAmps = 'N/A';
                
                if (dropdown) {
                    const selectedOption = dropdown.options[dropdown.selectedIndex];
                    if (selectedOption) {
                        // Extract panel amps from the option text (format: "Product Name (X amps)")
                        const match = selectedOption.textContent.match(/\((\d+(?:\.\d+)?)\s*amps?\)/);
                        if (match) {
                            panelAmps = `${match[1]} amps`;
                        } else {
                            // Fallback: try to get from product database
                            const category = dropdown.getAttribute('data-category');
                            const productId = dropdown.value;
                            if (category && productId && window.applianceDatabase) {
                                const product = window.applianceDatabase.getProductById(productId, category);
                                if (product) {
                                    panelAmps = `${product.panel_amps_240v || 0} amps`;
                                }
                            }
                        }
                    }
                ...
                // Create and insert the Panel Amps cell
                const panelAmpsCell = document.createElement('td');
                panelAmpsCell.textContent = panelAmps;
                row.insertBefore(panelAmpsCell, descriptionCell.nextSibling);
            }
        });
    ...
    getTotalCostRange() {
        ...
        rows.forEach(row => {
            ...
            if (productSelector && productSelector.value) {
                ...
                // Fallback: try to parse cost from the cost cell (column index 3 in displayed table, 4 in printed table)
                const costCell = row.cells[4] || row.cells[3];
                if (costCell) {
